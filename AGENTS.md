...
3. Codex CLI 长任务优先后台运行并监控；如果只能前台运行，也必须执行同样的心跳记录要求。
4. 每 2 分钟检查一次 Codex CLI 状态。
5. 每 5 分钟更新 PROGRESS.md，并追加 logs/codex-run.log；前台运行超过 5 分钟也必须记录“已运行 X 分钟/仍在运行/无新日志或最近输出摘要”。
6. 每完成一个小点，必须记录完成时间。
...
Codex CLI 长任务优先后台运行并监控；如果只能前台运行，也必须按同样频率写心跳日志。
...
- 如果任务在前台运行导致无法实时写文件，恢复控制权后必须第一时间补写 `logs/codex-run.log`，记录前台运行总时长、最后可见输出、是否被 Ctrl+C 中断、是否有产出文件变化、下一步动作。
- 如果前台运行超过 5 分钟但没有心跳日志，应视为监控规则未执行，必须在 `TASK_HANDOFF.md` 记录一次“前台心跳缺失”中断/偏差记录。
...
- 最近输出摘要
- 下一步动作
- 运行形态（foreground/background）
- 已运行时长（例如 `elapsed=6m`）
- 是否有新输出（例如 `no_new_output=true`）
