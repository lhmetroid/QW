...
- 每个 cron tick 在独立隔离会话里运行：读状态文件 → 只做第一个未完成任务 → 用中文写日志 → 更新状态 → 结束；下一 tick 继续。
...
- 每个 tick 只做 `TASKS.md` 第一个未完成任务，做完即结束，下一 tick 自动继续。
...
5. 不在会话内死等重试：本轮失败就如实记录后结束，由 gateway 的 cron 周期（如 every 20m）在下一轮自动重试——这正是循环存在的意义，断网不丢任务。
