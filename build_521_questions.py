...
_yy, _mm, _dd = TARGET_DATE.split("-")
CODEX_LOGS_DIR = Path.home() / ".codex" / "sessions" / _yy / _mm / _dd
...
def parse_gemini_logs() -> list[dict]:
    ...
    for folder in GEMINI_LOGS_DIR.iterdir():
        if not folder.is_dir():
            continue
        # Older sessions use overview.txt; newer ones use transcript.jsonl (same schema)
...
<head>
  ...
  <title>2026-05-21 智能助手多端提问汇总记录</title>
...
def main():
    print(f"--- 启动 {TARGET_DATE} 提问记录生成服务 ---")
