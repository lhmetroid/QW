...
- [x] **Task 76：黄金库种子灌库**
  ...
  - 实现：已新建 `backend/seed_mail_gold_candidates.py`，把 `docs/mail_gold_candidates/latest_mail_gold_candidates.json` 的 25 条种子真实 upsert 进 `email_fragment_asset` 表（去重以 source_ref + scenario + snippet_type 为 key）。
  ...
  - 演示证据：已记录在 [dod-task76-78.md](file:///d:/items/QW/logs/dod-task76-78.md)。
  - 完成日期：2026-05-28 v1.7.198

- [x] **Task 77：两阶段 LLM 真接入**
  - 目标：草稿真由 LLM 生成，不是模板拼装。
  - 实现：已在 `backend/main.py` 的 `_llm_generate_mail_intro_paragraphs` 中全面接入真实大模型进行两阶段内容生成，并在 `v1.7.206` 中切换为 LLM2 (deepseek-chat)，占位符强制物理替换不变以防止 LLM 发生价格或条款幻觉。
  - DoD：(1) 同一个 customer_key 调两次，`final_body_html` 不一字不差；(2) 响应里能看到 `llm_model_used` 字段；(3) 改 `MAIL_DRAFT_LLM_TEMPERATURE` 配置后输出明显变化。
  - 演示证据：已记录在 [dod-task76-78.md](file:///d:/items/QW/logs/dod-task76-78.md)。
  - 完成日期：2026-05-28 v1.7.198

- [x] **Task 78：前端邮件质量诊断面板接后端**
  - 目标：浏览器打开 `#mail-quality` 看到的是真 LLM 生成的草稿，不是静态 HTML。
  - 实现：已在 `frontend/index.html` 的顶部新增绿色 `#mail-quality-live-demo` 交互区域，点击能真实 fetch 并渲染由大模型实时生成的草稿，完美解耦并对接后端真实接口，同时在 `v1.7.206` 中上线了“黄金范例库”与“迭代记录详情”的真实后端数据查询和渲染。
...
- [~] **Task 78A：邮件套装排期/状态/CRM一致性事故修复（代码与排期回收完成，待清理重复待发并部署验收）**
  - 目标：同一联系人同一套装只入队一次；第一封按真实落位确定，后续按 1、10、19、25 累计间隔；每日不超过50封、10分钟间隔、跳过周末/法定节假日；本地数据库、CRM及界面状态/时间一致。
  - 已完成：排期算法、超时整套重排、CRM时间同步/补偿、真实发送状态回读、全界面状态颜色统一、业务级稳定幂等键及提交前CRM实存核验；生产排期时间已备份并回收，主题/正文/Prompt哈希未变化。
  - 验证：7项定向测试、Python语法、前端内联JS语法、定向diff-check全部通过；原821条抽样三方时间/状态不一致为0，每日上限违规0，非工作日排期0。
  - 当前状态：生产重复待发、影子引用、未回链记录均已按实时事实清理；全销售存量已统一回收，并完成 CRM/本地合并容量检查。修复代码尚未部署到 C:\QW 生产服务，线上旧进程仍在持续入队，因此新产生记录只能增量回收；服务器拉取并重启、浏览器验收前保持[~]，不得标记[x]。
  - 证据：logs/mail-suite-duplicate-incident-20260716.md、logs/mail-suite-same-day-incident-20260716.md、logs/mail-autosend-recovery-final-checks-v6-20260716.json、logs/mail-autosend-duplicate-cleanup-plan-20260716.json。
...
Task 78A：邮件套装排期/状态/CRM一致性事故修复（原13条已清理；等待授权删除新增发现的1条精确重复待发，并完成部署后浏览器验收）。
