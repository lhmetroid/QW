...
- 最终定向校验：三方异常0、十分钟冲突0、超过每日50封0、AI非工作日0、最大日量50；主题/正文/Prompt哈希不变；7项回归、py_compile、前端JS、定向diff-check通过。### 2026-07-16 新生成/历史同步复核
- 结论：当前还不能宣布“后续与历史同步均无问题”。新代码回归通过但尚未部署；生产只读审计仍有1条新增精确重复待发和23条历史三方未闭环记录。
- 安全指标仍正常：业务键重复0、10分钟冲突0、每日超50封0、周末排期0、最大日量50。
- 本轮只读，未删除、未改排期、未写CRM、未触发发信。

## 2026-07-17 Task 78A 全销售套装一致性深度修复（待生产部署）

- 根因闭环：同一 CRM RowId 被大小写不同的两条 send_plan 引用、旧 SendId/RowId 关联优先级错误、界面依赖缓存状态、手工套装未使用销售级槽位账本、同步超时时未将整个剩余套装按真实首封重新锚定。
- 数据回收：取消 25 条本地影子 send_plan（CRM 物理邮件 0 删除）；全销售 CRM 待发按 RowId 一对一重排，本地未同步排期与 CRM 合并重排；主题/正文/Prompt 哈希未变化，未触发真实发送。
- 已验证快照：跨表业务重复 0、共享物理引用 0、重复联系人套装 0、SendId 重号 0、精确撞槽 0；合并口径超过每日50封 0、不足10分钟 0、非工作日 0、最大日量50；统一重排二次预演 changed=0。
- 运行态事实：生产旧进程仍持续新增待发，实时数量在验证期间继续增长；本地代码未部署到服务器 C:\QW，因此不能宣称后续新生成已彻底受保护。下一步必须服务器 git pull + 重启后端，再强刷 mail-suite 页面做何珺及全销售验收。
