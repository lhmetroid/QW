...
# Explicitly disable WeChat/WeCom background message polling
$env:ENABLE_ARCHIVE_POLLING = "false"

$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$LogsDir = Join-Path $Root "backend\logs"
$StartScript = Join-Path $Root "start_backend.ps1"
$LogViewerScript = Join-Path $Root "start_mail_backend_log_viewer.ps1"
...
Write-Host "Mail-only backend detached process started."
