...
## 2026-07-17 13:15:04 交接：Task 78A 代码完成、生产旧进程仍持续入队

- 已完成代码：RowId 优先的 CRM 实时状态/时间回读；同收件人套装业务幂等；首封真实落位后按 1/10/19/25 累计间隔；超时整套清空重排；所有入口共用每日50、10分钟、周末/节假日槽位；状态同步支持 WaitSend/Sending 并批量查 RowId；审计关联加入销售+客户+场景+封次+收件邮箱。
