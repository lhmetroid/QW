"""
修复 S07 预算收紧：
- 保留7个合格案例（S07-01~03/05~07/10）
- 替换3个：S07-04台历产品细节（非预算压力）、S07-08服务能力询问（是S08）、S07-09账款调整（非预算收紧）
"""
import os, sys, io, re
...
KEEP_SESSIONS = [
    ('wri_a28e29584d1a_d125', date(2023,  9, 14)),  # S07-01 展台预算压缩10W两方案
    ('wri_a28e29584d1a_d125', date(2023,  9, 11)),  # S07-02 展台价格多轮谈判
    ('wri_a28e29584d1a_d147', date(2022,  9,  1)),  # S07-03 差旅税率太高申请财务
    ('wri_a28e29584d1a_d126', date(2026,  4, 13)),  # S07-05 5000以内预算限制+比价
    ('wri_a28e29584d1a_d126', date(2026,  4,  9)),  # S07-06 不在今年预算+分期讨论
    ('wri_a28e29584d1a_d91',  date(2024, 10,  9)),  # S07-07 报价超预算跨部门商量
    ('wri_a28e29584d1a_d95',  date(2022,  8,  3)),  # S07-10 以旧供应商低价为依据压价
]

ALL_USED = set(KEEP_SESSIONS) | {
    ('wri_a28e29584d1a_d91',  date(2025,  9, 16)),  # S07-04 台历产品细节（非预算）
    ('wri_a28e29584d1a_d127', date(2023,  6,  7)),  # S07-08 服务能力询问（是S08）
    ('wri_a28e29584d1a_d166', date(2022,  6, 10)),  # S07-09 账款调整（非预算收紧）
}
...
def find_replacements(conn, exclude_keys, need=3):
...
with e.connect() as conn:
    ...
    replacements = find_replacements(conn, exclude_keys=ALL_USED, need=3)
    ...
    s07_start = content.find('\n---\n\n# S07 ')
    s08_start = content.find('\n---\n\n# S08 ')
    if s07_start==-1 or s08_start==-1:
        print('❌ 找不到S07/S08标记'); sys.exit(1)
    ...
    new_content = content[:s07_start] + '\n'.join(new_lines) + content[s08_start:]
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(new_content)

    print(f'\n完成！S07已更新。保留{len(keep_sessions)}+替换{len(replacements)}={len(all_sessions)}个')
