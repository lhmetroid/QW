"""
修复 S06 售后纠偏/客诉处理：
- 保留6个合格案例（S06-01~03/06/08/09）
- 替换4个不合格案例：S06-04跨14个月、S06-05话题混搭、S06-07混乱4条、S06-10过短回应随意
"""
import os, sys, io, re
...
KEEP_SESSIONS = [
    ('wri_a28e29584d1a_d166', date(2022,  9, 28)),  # S06-01 发票金额账号纠偏
    ('wri_a28e29584d1a_d126', date(2023,  7, 12)),  # S06-02 视频字幕质量反馈
    ('wri_a28e29584d1a_d166', date(2024,  7, 10)),  # S06-03 未收到PO订单纠纷
    ('wri_a28e29584d1a_d95',  date(2022,  6,  6)),  # S06-06 翻译账单记错两次
    ('wri_a28e29584d1a_d143', date(2024,  4, 29)),  # S06-08 翻译质量差要求重译
    ('wri_a28e29584d1a_d143', date(2024,  6, 27)),  # S06-09 slogan语法错误反馈
]

ALL_USED = set(KEEP_SESSIONS) | {
    ('wri_a28e29584d1a_d112', date(2022,  8, 11)),  # S06-04 跨14个月结尾混乱
    ('wri_a28e29584d1a_d127', date(2022,  7, 15)),  # S06-05 质量抱怨+催款混搭
    ('wri_a28e29584d1a_d127', date(2022,  7, 21)),  # S06-07 4条混乱引用无关话题
    ('wri_a28e29584d1a_d147', date(2023,  9,  7)),  # S06-10 4条过短回应随意
}
...
def find_replacements(conn, exclude_keys, need=4):
...
with e.connect() as conn:
    ...
    replacements = find_replacements(conn, exclude_keys=ALL_USED, need=4)
    ...
    if s06_start==-1 or s07_start==-1:
        print('❌ 找不到S06/S07标记'); sys.exit(1)
    ...
    new_content = content[:s06_start] + '\n'.join(new_lines) + content[s07_start:]
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(new_content)

    print(f'\n完成！S06已更新。保留{len(keep_sessions)}+替换{len(replacements)}={len(all_sessions)}个')
