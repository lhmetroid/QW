"""
修复 S12 转介绍请求（数据库标签=转介绍请求与存量裂变）：
- 保留3个：S12-01客户推荐前公司、S12-03推给同事登记、S12-06同事Fansha要报价
- 替换7个：S12-02跨15天、S12-04跨10天、S12-05跨3个月、S12-07/08/09太短、S12-10方向相反
"""
import os, sys, io, re
...
KEEP_SESSIONS = [
    ('wri_a28e29584d1a_d166', date(2023,  3,  1)),  # S12-01 客户主动推荐前公司（7条）
    ('wri_a28e29584d1a_d130', date(2022, 11, 17)),  # S12-03 推给同事+需要登记（3条）
    ('wri_a28e29584d1a_d166', date(2024,  1, 16)),  # S12-06 同事Fansha要找报价（3条）
]

ALL_USED = set(KEEP_SESSIONS) | {
    ('wri_a28e29584d1a_d133', date(2021, 12, 16)),  # S12-02 跨15天（2021-12-16至2021-12-31）
    ('wri_a28e29584d1a_d133', date(2022,  3, 15)),  # S12-04 跨10天（2022-03-15至2022-03-25）
    ('wri_a28e29584d1a_d141', date(2025,  3, 25)),  # S12-05 跨3个月（2025-03-25至2025-06-16）
    ('wri_a28e29584d1a_d125', date(2025,  6, 12)),  # S12-07 2条太短
    ('wri_a28e29584d1a_d143', date(2025,  1,  6)),  # S12-08 2条，反向推荐
    ('wri_a28e29584d1a_d164', date(2023, 12,  1)),  # S12-09 2条太短
    ('wri_a28e29584d1a_d125', date(2022,  5, 11)),  # S12-10 客户向销售推荐供应商
}
...
def find_replacements(conn, exclude_keys, need=7):
    ...
    if len(session_map) < need:
        kw_rows = conn.execute(text("""
            ...
            WHERE group_key IN (SELECT group_key FROM emp_groups)
              ...
              AND (content LIKE '%帮我推荐%' OR content LIKE '%推荐给你%'
                OR content LIKE '%转介绍%' OR content LIKE '%介绍给%'
                OR content LIKE '%介绍一下%' OR content LIKE '%推给你%'
                OR content LIKE '%推荐过去%' OR content LIKE '%有没有认识%'
                OR content LIKE '%引荐%' OR content LIKE '%转个单%'
                OR content LIKE '%帮忙推%' OR content LIKE '%给你推%'
                OR content LIKE '%我把你%' OR content LIKE '%可以推荐%'
                OR content LIKE '%你们引荐%')
              AND role IN ('sales', 'customer')
    ...
    for (gk, sd), s in session_map.items():
        ...
        first_role = msgs[0][2]
        if not has_s or not has_c or n < 3: continue   # S12销售发起，最低3条
...
with e.connect() as conn:
    ...
    new_content = content[:s12_start] + '\n'.join(new_lines)
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(new_content)

    print(f'\n完成！S12已更新。保留{len(keep_sessions)}+替换{len(replacements)}={len(all_sessions)}个')
