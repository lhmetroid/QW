"""
修复 S03 产品/服务细节对标：
- 保留4个合格案例（S03-04礼品本规格/S03-05本子logo细节/S03-08设计排版/S03-09AI面试细节）
- 替换6个不合格案例（视频执行、文件加密报价、杂乱对话、brochure报价、文件传递、RGB售后纠纷）
"""
import os, sys, io, re
...
KEEP_SESSIONS = [
    ('wri_a28e29584d1a_d91',  date(2025,  9, 16)),  # S03-04 礼品本子规格定制细节
    ('wri_a28e29584d1a_d91',  date(2024, 10, 14)),  # S03-05 礼品本子logo细节确认
    ('wri_a28e29584d1a_d125', date(2022,  9, 30)),  # S03-08 设计排版中文/德文建议
    ('wri_a28e29584d1a_d145', date(2026,  1, 26)),  # S03-09 AI面试系统功能细节询问
]

ALL_USED = set(KEEP_SESSIONS) | {
    ('wri_a28e29584d1a_d126', date(2024,  9, 28)),  # S03-01 视频格式执行过程（非S03）
    ('wri_a28e29584d1a_d82',  date(2022,  6, 27)),  # S03-02 文件加密密码/报价（非S03）
    ('wri_a28e29584d1a_d125', date(2023,  4, 10)),  # S03-03 杂乱混合对话
    ('wri_a28e29584d1a_d121', date(2025,  7, 16)),  # S03-06 brochure翻译报价（S01）
    ('wri_a28e29584d1a_d122', date(2025,  6, 10)),  # S03-07 方案文件传递（非S03）
    ('wri_a28e29584d1a_d121', date(2025, 11, 24)),  # S03-10 RGB颜色纠纷（S06）
}
...
with e.connect() as conn:
    ...
    s04_start = content.find('\n---\n\n# S04 ')
    if s03_start==-1 or s04_start==-1:
        print('❌ 找不到S03/S04标记'); sys.exit(1)
    ...
    new_content = content[:s03_start] + '\n'.join(new_lines) + content[s04_start:]
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(new_content)

    print(f'\n完成！S03已更新。保留{len(keep_sessions)}+替换{len(replacements)}={len(all_sessions)}个')
