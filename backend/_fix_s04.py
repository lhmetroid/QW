"""
修复 S04 价格竞争/比价：
- 保留8个合格案例（S04-01~03/05~09，均有明确议价或降价请求）
- 替换2个不合格案例：S04-04软件询价无比价行为（像S01）、S04-10口译类型比较（是S03非S04）
"""
import os, sys, io, re
...
KEEP_SESSIONS = [
    ('wri_a28e29584d1a_d125', date(2023,  9, 11)),  # S04-01 15万以内价格谈判
    ('wri_a28e29584d1a_d166', date(2023,  8, 17)),  # S04-02 翻译报价太高调整级别
    ('wri_a28e29584d1a_d127', date(2024, 10, 17)),  # S04-03 价格偏高+付款流程纠纷
    ('wri_a28e29584d1a_d159', date(2023,  1, 10)),  # S04-05 老客台历排版议价
    ('wri_a28e29584d1a_d126', date(2026,  4, 15)),  # S04-06 降价后质量担忧
    ('wri_a28e29584d1a_d147', date(2023,  9,  7)),  # S04-07 价格调整请求
    ('wri_a28e29584d1a_d164', date(2024,  9, 17)),  # S04-08 国外客户反馈价格过高
    ('wri_a28e29584d1a_d133', date(2024,  1, 11)),  # S04-09 经销商付款要便宜
]

ALL_USED = set(KEEP_SESSIONS) | {
    ('wri_a28e29584d1a_d126', date(2024,  7,  4)),  # S04-04 软件翻译询价（像S01）
    ('wri_a28e29584d1a_d165', date(2022,  3, 25)),  # S04-10 口译类型比较（是S03）
}
...
with e.connect() as conn:
    ...
    if s04_start==-1 or s05_start==-1:
        print('❌ 找不到S04/S05标记'); sys.exit(1)
    ...
    new_content = content[:s04_start] + '\n'.join(new_lines) + content[s05_start:]
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(new_content)

    print(f'\n完成！S04已更新。保留{len(keep_sessions)}+替换{len(replacements)}={len(all_sessions)}个')
