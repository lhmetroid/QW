"""
修复 S08 业务深度介绍：
- 保留4个合格案例（S08-01翻译质量要求/S08-02公司介绍/S08-04语种范围/S08-05服务历史了解）
- 替换6个：S08-03加急（S05）、S08-06翻译人员（S06）、S08-07改错字（S06）、
           S08-08跨7天4条、S08-09后天有人（S05）、S08-10印刷细节（S03）
"""
import os, sys, io, re
...
KEEP_SESSIONS = [
    ('wri_a28e29584d1a_d127', date(2022,  5, 20)),  # S08-01 翻译质量要求+翻译资源说明
    ('wri_a28e29584d1a_d127', date(2023,  6,  7)),  # S08-02 新公司需要公司介绍+PPT
    ('wri_a28e29584d1a_d143', date(2024,  3,  8)),  # S08-04 多语种服务能力询问
    ('wri_a28e29584d1a_d148', date(2025,  1, 10)),  # S08-05 新人了解服务历史和术语表
]

ALL_USED = set(KEEP_SESSIONS) | {
    ('wri_a28e29584d1a_d166', date(2024,  9, 26)),  # S08-03 加急翻译（是S05）
    ('wri_a28e29584d1a_d166', date(2024, 12, 11)),  # S08-06 翻译人员确认（是S06）
    ('wri_a28e29584d1a_d126', date(2022,  3, 28)),  # S08-07 改错字（是S06）
    ('wri_a28e29584d1a_d126', date(2024,  8, 21)),  # S08-08 4条跨7天
    ('wri_a28e29584d1a_d147', date(2023,  9,  5)),  # S08-09 后天有人（是S05）
    ('wri_a28e29584d1a_d121', date(2025,  9, 29)),  # S08-10 印刷细节（是S03）
}
...
with e.connect() as conn:
    ...
    s09_start = content.find('\n---\n\n# S09 ')
    if s08_start==-1 or s09_start==-1:
        print('❌ 找不到S08/S09标记'); sys.exit(1)
    ...
    new_content = content[:s08_start] + '\n'.join(new_lines) + content[s09_start:]
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(new_content)

    print(f'\n完成！S08已更新。保留{len(keep_sessions)}+替换{len(replacements)}={len(all_sessions)}个')
