"""
修复 S09 节气/开工问候：
- 保留4个合格案例（S09-01/02/05/07，纯当天开工问候）
- 替换6个：S09-03/04/06跨多月混搭、S09-08问候+催款、S09-09问候+业务混搭、S09-10单方触达跨年
"""
import os, sys, io, re
...
KEEP_SESSIONS = [
    ('wri_a28e29584d1a_d125', date(2023,  2,  2)),  # S09-01 开工大吉+展商手册
    ('wri_a28e29584d1a_d126', date(2024,  2, 18)),  # S09-02 龙年新年开工问候
    ('wri_a28e29584d1a_d125', date(2024,  2, 18)),  # S09-05 新年开工问候互动
    ('wri_a28e29584d1a_d148', date(2025,  2,  8)),  # S09-07 新年开工问候
]

ALL_USED = set(KEEP_SESSIONS) | {
    ('wri_a28e29584d1a_d165', date(2023,  1,  4)),  # S09-03 跨2个月混搭
    ('wri_a28e29584d1a_d141', date(2022,  3, 25)),  # S09-04 跨4个月三话题混搭
    ('wri_a28e29584d1a_d138', date(2024,  2, 18)),  # S09-06 跨5个月末尾加密文件
    ('wri_a28e29584d1a_d127', date(2022,  1,  4)),  # S09-08 问候+催款混搭
    ('wri_a28e29584d1a_d143', date(2025,  1,  3)),  # S09-09 问候+翻译业务混搭
    ('wri_a28e29584d1a_d144', date(2022,  3, 25)),  # S09-10 单方触达跨2022-2025
}
...
with e.connect() as conn:
    ...
    if s09_start==-1 or s10_start==-1:
        print('❌ 找不到S09/S10标记'); sys.exit(1)
    ...
    new_content = content[:s09_start] + '\n'.join(new_lines) + content[s10_start:]
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(new_content)

    print(f'\n完成！S09已更新。保留{len(keep_sessions)}+替换{len(replacements)}={len(all_sessions)}个')
